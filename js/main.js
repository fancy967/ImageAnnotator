...
$(document).ready(function () {
  ...
  $('#input_text').change(function () {
    $(g_curDIV).attr('data-text', $('#input_text').val());
    for (var i = 0, l = g_marksXML.object.length; i < l; i++) {
      if (g_marksXML.object[i].id == g_curDIV.id) {
        g_marksXML.object[i].text = $(g_curDIV).attr('data-text');
      }
    }
    updateMarks();
    var b_id = g_curDIV.id;
    $('#div_marks_list button.btn-default[bndbox="' + b_id + '"]').html($('#input_text').val());
  });
  ...
  $('#div_container').on("contextmenu", '.img-bndbox', function (e) {
    if (cDiv) return;
    g_curDIV = this;
    $('#btn_del_confirm').click();
  });
  ...
  $(window).mouseup(function (e) {
    if ($('#div_toolkit').is(":visible") && $(e.target).parents("#div_toolkit").length != 1
      && $('#label_mark').html() == 'Null' && !$('#input_text').is(':focus')) {
      g_curDIV = null;
      // $('#div_toolkit').fadeOut('fast');
      $('#div_toolkit').hide();
    }
    ...
    $('#div_marks_list').append('<div class="btn-group"><button type="button" bndbox="bndbox-' + g_bndBoxCnt +
      '" class="btn btn-default btn-sm active">Null</button><button type="button" bndbox="bndbox-' + g_bndBoxCnt +
...
function showToolkit(x, y, showDropdown) {
  ...
  $('#div_toolkit .input-group').removeClass('dropup');
  $('#input_text').val($(g_curDIV).attr('data-text'));
